...
node_st *DGFTiactions(node_st *node) { return node; }

node_st *DGFTiphase(node_st *node) { return node; }

node_st *DGFTitraversal(node_st *node) { return node; }

node_st *DGFTitravdata(node_st *node) { return node; }
...
node_st *DGFTinode(node_st *node) {
    GeneratorContext *ctx = globals.gen_ctx;
    curr_node = node;
    OUT_START_FUNC("struct ccn_node *DEL%s(struct ccn_node *arg_node)",
...
node_st *DGFTattribute(node_st *node) {
    ...
    if (ATTRIBUTE_TYPE(node) == AT_string) {
        OUT_FIELD("MEMfree(arg_node->data.N_%s->%s)",
...
node_st *DGFTienum(node_st *node) { return node; }

node_st *DGFTid(node_st *node) { return node; }

node_st *DGFTilifetime(node_st *node) { return node; }

node_st *DGFTlifetime_range(node_st *node) { return node; }

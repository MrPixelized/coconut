...
void checkUID(node_st *id) {
    ...
    for (unsigned int i = 0; i < sizeof(uid_reserved) / sizeof(uid_reserved[0]);
         i++) {
...
node_st *CEXitravdata(node_st *node) {
    ...
    if (poss_conflict != NULL) {
        struct ctinfo info = {.filename = globals.filename,
                              .first_line = ID_ROW(ITRAVDATA_NAME(node))};
        CTIobj(CTI_ERROR, false, info,
               "Name(%s) is used multiple times in travdata.",
               ID_ORIG(ITRAVDATA_NAME(node)));
        struct ctinfo conf_info = {.filename = globals.filename,
                                   .first_line = ID_ROW((poss_conflict))};
        CTIobj(CTI_NOTE, true, conf_info, "Used here as well.");
    } else {
        trav_data_names =
            STadd(trav_data_names, ITRAVDATA_NAME(node), ITRAVDATA_NAME(node));
    }
    ...
    if (ITRAVDATA_TYPE(node) == AT_link_or_enum) {
        ...
        if (ste_entry == NULL) {
            CTI(CTI_ERROR, true,
                "Could not find type reference for travdata entry.");
        } else {
            if (NODE_TYPE(ste_entry) == NT_INODE ||
                NODE_TYPE(ste_entry) == NT_INODESET) {
...
node_st *CEXchild(node_st *node) {
    ...
    if (!ref) {
        ...
        CTIobj(CTI_ERROR, true, info, "Child type %s is not defined.",
               ID_ORIG(CHILD_TYPE_REFERENCE(node)));
        return node;
    }
    ...
    if (STlookup(node_ste, CHILD_NAME(node)) == NULL) {
        ...
        CTIobj(CTI_ERROR, true, info,
               "Double declaration of attribute/child name: %s\n",
               ID_ORIG(CHILD_NAME(node)));
    }
    TRAVchildren(node);
    return node;
...
node_st *CEXsetreference(node_st *node) {
    ...
    if (!ref_node) {
        ...
        CTI(CTI_ERROR, true,
            "Set reference is not a reference to a nodeset: %s\n",
            ID_ORIG(SETREFERENCE_REFERENCE(node)));
    }
    TRAVchildren(node);
    return node;
...
node_st *CEXsetliteral(node_st *node) {
    ...
    if (entry == NULL) {
        ...
        if (NODE_TYPE(entry) != NT_INODE) {
            ...
            id_to_info(SETLITERAL_REFERENCE(node), &info);
            CTIobj(CTI_ERROR, true, info,
                   "Set member is not a valid node: %s\n",
                   ID_ORIG(SETLITERAL_REFERENCE(node)));
        }
    }
    TRAVchildren(node);
...
node_st *CEXattribute(node_st *node) {
    ...
    if (ATTRIBUTE_TYPE(node) == AT_link_or_enum) {
        ...
        if (ste_entry == NULL) {
            ...
            CTIobj(CTI_ERROR, true, info,
                   "Could not find type reference for attribute with type %s.",
                   ID_ORIG(ATTRIBUTE_TYPE_REFERENCE(node)));
        } else {
            if (NODE_TYPE(ste_entry) == NT_INODE ||
                NODE_TYPE(ste_entry) == NT_INODESET) {
                ...
                CTI(CTI_ERROR, true, "Invalid type in attribute type: %s",
                    ID_ORIG(ATTRIBUTE_TYPE_REFERENCE(node)));
            }
        }
    }
...
node_st *CEXienum(node_st *node) {
    for (unsigned int i = 0;
         i < sizeof(preserved_enum_prefix) / sizeof(preserved_enum_prefix[0]);
         i++) {
        if (STReq(preserved_enum_prefix[i], ID_UPR(IENUM_IPREFIX(node)))) {
            CTI(CTI_ERROR, true, "Enum prefix %s is reversed.",
                preserved_enum_prefix[i]);
        }
    }
...
node_st *CEXilifetime(node_st *node) { return node; }

node_st *CEXlifetime_range(node_st *node) { return node; }
